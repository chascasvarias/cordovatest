...
function Calculate() {
    ...
    if (boolErrorCheck) alert("Empty Field: Age at Retirement");
    boolErrorCheck = isEmpty(formService);
    if (boolErrorCheck) alert("Empty Field: Years of Service");
    boolErrorCheck = isEmpty(formFinAvgMoComp);
    if (boolErrorCheck) alert("Empty Field: Final Average Compensation");
    ...
    if (!boolErrorCheck) {
        ...
        switch (tierType) {
        case "General tier 1":
            ageFactor = gettier1Factor();
            break;
        case "General tier 2":
            ageFactor = gettier2Factor();
            break;
        case "Safety":
            ageFactor = getSafetyFactor();
            break;
        case "Safety3":
            ageFactor = getSafety3Factor();
            break;
        case "tier2555":
            ageFactor = gettier2555Factor();
            break;
        case "tier2755":
            ageFactor = gettier2755Factor();
            break;
        case "tier360":
            ageFactor = gettier360Factor();
            break;
        case "tier2055":
            ageFactor = gettier2055Factor();
            break;
        case "tier1665":
            ageFactor = gettier1665Factor();
            break;
        case "safety355":
            ageFactor = getsafety355Factor();
            break;
        ...
        default:
            alert("No tier Type Selected!"); // Added for security, but not required (using the CHECKED command in the radio button to force an initial selection)
            break;
        }
        ...
        if (EstimatedBenefit > parseFloat(formFinAvgMoComp)) {
            ...
            alert(" Your monthly allowance would be " + "$" + (document.RetBenCalc.FinAvgMoComp.value) + " because the maximum monthly allowance you can receive is 100% of your Average Monthly Compensation.")
        }
...
function gettierType() {
    ...
    for (var i = 0; i < selectedtype.tiersRet.length; i++) {
        if (selectedtype.tiersRet[i].checked) {
            break
        }
    }
    return selectedtype.tiersRet[i].value;
}
    ...
    if (isNaN(input)) {
        ...
        if (document.RetBenCalc.Age.value.length > 6) {
            ...
            if (!inRange(input, 38, 85)) {
                alert("If you are a General member, please enter an age of 48 and older. If you are a Safety member, please enter an age of 38 and older.");
                return false;
            }
            return true;
